...
forecast_start_day <- as_date("2018-09-01")
last_forecast_start_day <- as_date("2020-01-01")
days_between_foreasts <- 7
forecast_horizon <- 16
#forecast_start<- as_date("2018-12-31")
#forecast_start<- as_date("2019-12-31")
#forecast_start<- as_date("2019-12-31")
start_days_vector <- start_day
forecast_start_day_vector <- forecast_start
while(forecast_start <= last_forecast_start_day){
  start_day <- forecast_start
  forecast_start <- forecast_start + days(days_between_foreasts)
  start_days_vector <- c(start_days_vector, start_day)
  forecast_start_day_vector <- c(forecast_start_day_vector, forecast_start)
}

forecast_days_vector <- rep(forecast_horizon, length(start_days_vector))
forecast_days_vector[1] <- 0

forecasting_timings <- data.frame(start_days_vector,
                                  forecast_start_day_vector,
                                  forecast_days_vector)
